...
public class SessionController {
    ...
    /**
     * Staff đánh dấu xe vẫn đậu tại trạm (bắt đầu tính phí đỗ xe)
     * POST /api/sessions/{sessionId}/staff/still-parked
     */
    @PostMapping("/{sessionId}/staff/still-parked")
    public ResponseEntity<APIResponse<Long>> staffMarkStillParked(@PathVariable Long sessionId,
                                                                   HttpServletRequest httpServletRequest) {
        String header = httpServletRequest.getHeader("Authorization");
        String token = jwtUtil.getTokenFromHeader(header);
        Long staffId;
        try {
            staffId = jwtUtil.getUserIdByTokenDecode(token);
            Long result = ((swp391.code.swp391.service.SessionServiceImpl) sessionService).staffMarkStillParked(sessionId, staffId);
            return ResponseEntity.ok(APIResponse.success("Đã đánh dấu xe vẫn đậu tại trạm, bắt đầu tính phí đỗ xe", result));
        } catch (ParseException | JOSEException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(APIResponse.error("Token parsing error"));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(APIResponse.error(e.getMessage()));
     ...
     * POST /api/sessions/{sessionId}/staff/left
     */
    @PostMapping("/{sessionId}/staff/left")
    public ResponseEntity<APIResponse<Long>> staffMarkLeft(@PathVariable Long sessionId,
                                                           HttpServletRequest httpServletRequest) {
        String header = httpServletRequest.getHeader("Authorization");
        String token = jwtUtil.getTokenFromHeader(header);
        Long staffId;
        try {
            staffId = jwtUtil.getUserIdByTokenDecode(token);
            Long result = ((swp391.code.swp391.service.SessionServiceImpl) sessionService).staffMarkLeft(sessionId, staffId);
            return ResponseEntity.ok(APIResponse.success("Đã xác nhận xe rời trạm, slot được giải phóng", result));
        } catch (ParseException | JOSEException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(APIResponse.error("Token parsing error"));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(APIResponse.error(e.getMessage()));
    ...
    /**
     * Lấy danh sách sessions đang chờ staff xác nhận
     * GET /api/sessions/waiting-staff-decision
     */
    @GetMapping("/waiting-staff-decision")
    public ResponseEntity<APIResponse<List<SessionDTO>>> getSessionsWaitingStaffDecision() {
        try {
            List<SessionDTO> sessions = sessionService.getAllSessions().stream()
                    .filter(s -> s.getStatus() != null &&
                            (s.getStatus().toString().equals("WAITING_STAFF_DECISION") ||
                             s.getStatus().toString().equals("STILL_PARKED")))
                    .collect(java.util.stream.Collectors.toList());
            return ResponseEntity.ok(APIResponse.success("Lấy danh sách sessions chờ staff xác nhận thành công", sessions));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(APIResponse.error(e.getMessage()));
        }
    }
